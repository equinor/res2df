...
def test_complump_in_welopen():
    """Tests that WELOPEN can refer to COMPLUMP numbers in order
    to OPEN or SHUT connections. And that correct Errors are raised
    if the COMPLUMP/WELOPEN input is not correctly defined"""
    schstr = """
DATES
    1 JAN 2000 /
/
COMPDAT
   'OP1' 74 135 7 7 'OPEN' 1* 20.2754986 0.1778 1797.38834 0 1* 'Y' 10.2702486 /
   'OP1' 74 135 8 8 'OPEN' 1* 227.999766 0.1778 20450.5141 0 1* 'Y' 10.8628435 /
   'OP1' 74 136 8 8 'OPEN' 1* 34.2261884 0.1778 3071.71533 0 1* 'Y' 10.8932574 /
   'OP1' 74 136 9 9 'OPEN' 1* 155.563774 0.1778 13843.9899 0 1* 'Y' 10.4613352 /
   'OP1' 74 137 10 10 'OPEN' 1* 126.1235646 0.1778 4352.9464 0 1* 'Y' 10.3458792 /
   'OP2' 34 158 3 3 'OPEN' 1* 126.1235646 0.1778 4352.9464 0 1* 'Y' 10.3458792 /
/
COMPLUMP
    'OP1' 74 135 7 8 1 /
    'OP1' 74 136 8 8 2 /
...
/
    """
    deck = EclFiles.str2deck(schstr)
    df_compdat = compdat.deck2dfs(deck)["COMPDAT"]
    check_columns = ["WELL", "I", "J", "K1", "K2", "OP/SH", "DATE"]
    expected_dframe = pd.DataFrame(
        data=[
            ["OP1", 74, 135, 7, 7, "OPEN", "2000-01-01"],
            ["OP1", 74, 135, 8, 8, "OPEN", "2000-01-01"],
            ["OP1", 74, 136, 8, 8, "OPEN", "2000-01-01"],
            ["OP1", 74, 136, 9, 9, "OPEN", "2000-01-01"],
            ["OP1", 74, 137, 10, 10, "OPEN", "2000-01-01"],
            ["OP2", 34, 158, 3, 3, "OPEN", "2000-01-01"],
            ["OP1", 74, 136, 8, 8, "SHUT", "2000-02-01"],
            ["OP1", 74, 136, 9, 9, "SHUT", "2000-02-01"],
            ["OP1", 74, 135, 7, 7, "SHUT", "2000-02-01"],
            ["OP1", 74, 135, 8, 8, "SHUT", "2000-02-01"],
            ["OP2", 34, 158, 3, 3, "SHUT", "2000-02-01"],
        ],
        columns=check_columns,
    )
    df_compdat.DATE = df_compdat.DATE.astype(str)
    pd.testing.assert_frame_equal(
        df_compdat[check_columns], expected_dframe, check_like=True
    )

    schstr = """
COMPDAT
   'OP1' 74 135 7 8 'OPEN' 1* 20.2754986 0.1778 1797.38834 0 1* 'Y' 10.2702486 /
/
COMPLUMP
    'OP1' 74 135 8 7 1 /
/
...
/
"""
    with pytest.raises(ValueError, match="K2 must be equal to or greater than K1"):
        compdat.deck2dfs(EclFiles.str2deck(schstr))
...
/
"""
    with pytest.raises(ValueError, match="C2 must be equal or greater than C1"):
        compdat.deck2dfs(EclFiles.str2deck(schstr))
