...
def bulk_upload(eclpath, metadata_path):
    """Bulk uploads every module to sumo with metadata

        eclpath (str): path to eclipse datafile
        metadata_path (str): path to metadata file
    """
    compdat(eclpath, metadata_path)
    equil(eclpath, metadata_path)
    faults(eclpath, metadata_path)
    fipreports(eclpath, metadata_path)
    grid(eclpath, metadata_path)
    nnc(eclpath, metadata_path)
    pvt(eclpath, metadata_path)
    rft(eclpath, metadata_path)
    satfunc(eclpath, metadata_path)
    summary(eclpath, metadata_path)