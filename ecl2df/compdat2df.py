...
compsegs_df = pd.DataFrame(columns=['date', 'well', 'i', 'j', 'k',
                                    'branch', 'distance_start',
                                    'distance_end'])
#, 'direction','end_ijk', 'center_depth', 'thermal_length', 'segment_number'])

welsegs_df = pd.DataFrame(columns=['date', 'well', 'depth', 'length',
                                   'wellbore_volume', 'info_type',
                                   'pressure_components', 'flow_model',
                                   'top_x', 'top_y', 'segment', 'branch',
                                   'join_segment', 'segment_length',
                                   'depth_change', 'diameter',
                                   'roughness'])
...
def deck2compdatsegsdfs(eclfiles):
    ...
    compdat_df = pd.DataFrame(columns=['date', 'well', 'i', 'j', 'k',
                                          'state', 'sat_table',
                                                    'connectiontrans', 'diam',
                               'kh', 'skin', 'd_factor',
                                                                         'direction', 'pr'])
    compsegs_df = pd.DataFrame(columns=['date', 'well', 'i', 'j', 'k',
                                           'branch', 'distance_start',
        ...
        if kw.name == "DATES":
            ...
            for rec in kw:
                for layer in range(int(rec["K1"][0]), int(rec["K2"][0])+1):
                    compdat_df.loc[len(compdat_df)] = [datestr, rec['WELL'][0],
                                                       rec['I'][0], rec['J'][0],
                                                       str(layer), rec['STATE'][0],
                                                       rec['SAT_TABLE'][0],
            ...
            for recidx in range(1,len(kw)):
                rec = kw[recidx]
                compsegs_df.loc[len(compsegs_df)] = [datestr, well, str(int(rec['I'][0])), str(int(rec['J'][0])), str(int(rec['K'][0])),  str(int(rec['BRANCH'][0])), rec['DISTANCE_START'][0], rec['DISTANCE_END'][0]]#q, rec['DIRECTION'][0], rec['END_IJK'][0], rec['CENTER_DEPTH'][0], rec['THERMAL_LENGTH'][0], rec['SEGMENT_NUMBER'][0]]
            ...
            for recidx in range(1, len(kw)):
                ...
                welsegs_df.loc[len(welsegs_df)] = [datestr, well, depth, length, wellbore_volume, info_type, pressure_components, flow_model, top_x, top_y, rec['SEGMENT1'][0], rec['BRANCH'][0], rec['JOIN_SEGMENT'][0], rec['SEGMENT_LENGTH'][0], rec['DEPTH_CHANGE'][0], rec['DIAMETER'][0], rec['ROUGHNESS'][0]]


    striptables=True
    if striptables:
        compdat_df.drop(['state', 'sat_table', 'diam', 'skin', 'd_factor', 'direction', 'pr'], inplace=True, axis=1)
        welsegs_df.drop(['wellbore_volume', 'info_type', 'pressure_components', 'flow_model', 'top_x', 'top_y', 'diameter', 'roughness'], inplace=True, axis=1)
...
def postprocess():
    ...
    compdatsegwel_icd_df = pd.merge(compsegs_icd_df, welsegs_df, on=['date', 'well', 'branch'])
    ...
    alldata_icd = pd.merge(compdatsegwel_icd_df, welsegs_df, on=['date', 'well', 'segment'])
