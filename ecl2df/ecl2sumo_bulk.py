...
def bulk_upload(eclpath, metadata_path, include:List = None, options = options):
    ...
    """
    subfunc_list = [compdat, equil, faults, fipreports, grid, nnc, pillars, pvt, rft, satfunc, summary]

    for subfunc in subfunc_list:
        if include is None or subfunc.__name__ in include:
            sig_items = signature(subfunc).parameters.items()
            filtered_options = {name: options[name] for name, param in sig_items if param.kind is not Parameter.empty and name not in {"eclpath", "metadata_path"}}
